...
const SongService = {
  ...
  listaCanciones: (nPage, itemsPerPage, filter) => {
    return new Promise((resolve, reject) => {
      ...
      if (start < songs.length) {
        if (filter) llista = songs.filter(filter);
        else llista = songs;

        res = {
          status: 200,
          message: 'OK!',
          canciones: llista.slice(start, end),
          nPages: Math.ceil(llista.length / itemsPerPage),
        };
      }
  ...
  listaArtistas: (nPage, itemsPerPage, filter, order) => {
    return new Promise((resolve) => {
      ...
      if (filter) llista = uniqeGroups.filter(filter);
      else llista = uniqeGroups;

      if (order) llista = llista.sort(order);
